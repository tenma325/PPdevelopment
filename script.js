(() => {
  ...
  /* --------------------------------------------------------------------------
     Connection lines: Central Concept Tile -> Geometry Museum
     SVG paths drawn in document coordinates from the bottom of the
     central card down to the top of each museum card.
  -------------------------------------------------------------------------- */
  const connectionLayer = document.querySelector('.connection-layer');
  const connectionSvg = connectionLayer ? connectionLayer.querySelector('.connection-svg') : null;
  const connectionPaths = connectionLayer ? connectionLayer.querySelector('.connection-paths') : null;

  function computeConnectionPaths() {
    if (!connectionLayer || !connectionSvg || !connectionPaths) return;

    const source = document.querySelector('.central-card');
    const targets = document.querySelectorAll('.museum-card');
    if (!source || !targets.length) return;

    const docWidth = document.documentElement.scrollWidth;
    const docHeight = document.documentElement.scrollHeight;
    connectionSvg.setAttribute('viewBox', `0 0 ${docWidth} ${docHeight}`);
    connectionSvg.setAttribute('width', docWidth);
    connectionSvg.setAttribute('height', docHeight);
    connectionLayer.style.height = docHeight + 'px';

    const sourceRect = source.getBoundingClientRect();
    const sourceX = sourceRect.left + sourceRect.width / 2 + window.scrollX;
    const sourceY = sourceRect.bottom + window.scrollY;

    connectionPaths.innerHTML = '';
    targets.forEach((target, i) => {
      const r = target.getBoundingClientRect();
      const targetX = r.left + r.width / 2 + window.scrollX;
      const targetY = r.top + window.scrollY;

      // Bezier curve: control points biased so the path drops vertically,
      // then curves to the target. Gives a "stem" feel.
      ...
      path.dataset.targetIndex = i;
      connectionPaths.appendChild(path);
    });
  }

  if (connectionLayer && connectionSvg) {
    // Compute paths on load (after layout settles)
    requestAnimationFrame(() => {
      computeConnectionPaths();
      // Reveal paths immediately so the connection is visible on first paint
      requestAnimationFrame(() => {
        const paths = connectionPaths.querySelectorAll('path');
        paths.forEach((p) => p.classList.add('revealed'));
      });
    });

    // Also recompute and re-reveal on scroll into the museum section
    ...
    }, { threshold: 0.02, rootMargin: '300px 0px 0px 0px' });

    const museumSection = document.querySelector('.museum-section');
    if (museumSection) {
      pathObserver.observe(museumSection);
    }
